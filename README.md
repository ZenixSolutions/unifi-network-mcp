...
3. Ask your assistant something like _"list the clients on my UniFi network"_.

## Configuration

| Variable            | Required | Meaning                                                                                                                                                                                                                                         |
| ------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `UNIFI_CONSOLE_URL` | yes      | Console base URL, e.g. `https://192.168.1.1`. The integration path (`/proxy/network/integration`) is appended automatically. The documented [Site Manager Connector](https://developer.ui.com) base on `api.ui.com` also works — no VPN needed. |
| `UNIFI_API_KEY`     | yes      | API key from unifi.ui.com. Sent as the `X-API-KEY` header.                                                                                                                                                                                      |
| `UNIFI_INSECURE`    | no       | Set to `1` to accept the console's self-signed TLS certificate. A warning is printed on every start. See [SECURITY.md](SECURITY.md) for the trade-off.                                                                                          |
